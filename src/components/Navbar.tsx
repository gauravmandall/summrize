    ...
    <header className="sticky inset-x-0 top-0 z-50 w-full bg-white border-b dark:bg-[#121212] h-14 border-border">
      <div className="relative items-center justify-between hidden h-full px-8 md:flex">
        <div className="flex items-center select-none gap-x-4">
            ...
            >
              ...
              <SheetContent side="left" className="w-full max-w-xs">
                ...
                <div className="flex flex-col items-start w-full mt-8 gap-y-2">
                  ...
                  <Button
                    variant={activePath === "/translator" ? "default" : "ghost"}
                    onClick={() => handleLink(false)}
                    className="justify-start w-full gap-x-2"
                  >
                    <Link
                  ...
                  <Button
                    variant={
                      activePath === "/grammar-check" ? "default" : "ghost"
                    }
                    onClick={() => handleLink(false)}
                    className="justify-start w-full gap-x-2"
                  >
                  ...
                  <Button
                    variant={activePath === "/chat" ? "default" : "ghost"}
                    onClick={() => handleLink(false)}
                    className="justify-start w-full gap-x-2"
                  >
                    <Link
                  ...
                  <Button
                    variant={
                      activePath === "/audio-generator" ? "default" : "ghost"
                    }
                    onClick={() => handleLink(false)}
                    className="justify-start w-full gap-x-2"
                  >
                  ...
                  <Button
                    variant={activePath === "/premium" ? "default" : "ghost"}
                    asChild
                    className="justify-start w-full"
                    onClick={() => handleLink(false)}
                  >
          ...
          <Link href="/" className="relative group">
            <Image
              ...
              height={50}
              alt="Summrize Logo"
              className="object-cover w-auto h-6 transition-all duration-700 ease-out dark:hidden"
            />
            <Image
              src="/logo-dark.svg"
              width={50}
              height={50}
              alt="Summrize Logo"
              className="hidden object-cover w-auto h-6 dark:block"
            />
          </Link>
        </div>
        ...
        <div className="absolute top-0 z-0 flex items-center justify-center py-3.5 mx-auto -translate-x-1/2 left-1/2">
          <span className="text-sm font-semibold text-center capitalize md:text-xl">
            {pathname === "/"
              ...
              : pathname === "/chat"
              ? "AI Chat"
              : pathname === "/grammar-check"
              ? "Grammar Check"
              : pathname === "/audio-generator"
              ? "Audio Generator"
              : "Summrize"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {!isLoaded && !user && <Skeleton className="w-16 h-9" />}
          {!isLoaded && user && <Skeleton className="rounded-full w-9 h-9" />}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between h-full px-4 md:hidden">
        <div className="flex items-center pl-4 select-none">
          <Link href="/" className="relative group">
            <Image
              ...
              height={50}
              alt="Summrize Logo"
              className="object-cover w-auto h-6 dark:hidden group-hover:rotate-[360deg] duration-700 ease-out transition-all"
            />
            <Image
              src="/logo-dark.svg"
              width={50}
        ...
        <div className="flex items-center gap-3">
          ...
          {!isLoaded && user && <Skeleton className="w-8 h-8 rounded-full" />}
